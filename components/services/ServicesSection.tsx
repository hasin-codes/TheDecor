    ...
    >
      ...
      >
        <div className="w-full h-full flex flex-col justify-center">
          {/* Header */}
          <div className="flex flex-col items-center text-center mb-2 md:mb-4 lg:mb-6 shrink-0 gap-2 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 pt-24 sm:pt-28 md:pt-32">
          ...
          <div className="shrink-0 mb-6 md:mb-8 flex justify-center gap-2">
            {services.map((_, i) => (
              <div
                key={i}
          ...
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-16 flex-1 items-center lg:items-center overflow-hidden pb-16 sm:pb-20 pt-8 sm:pt-10">
            {/* Left Column: List */}
              ...
              {services.map((service, i) => {
                ...
                return (
                  <div
                    key={service.id}
                    className={`
                      group relative px-4 py-4 sm:px-5 sm:py-5 md:px-6 md:py-6 rounded-xl md:rounded-3xl transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] border
                      ${isActive
                        ? 'bg-brandRed text-white border-transparent shadow-[0_0_30px_rgba(227,27,35,0.3)] scale-[1.01] md:scale-[1.02] z-10'
            ...
            <div className="hidden lg:flex w-full lg:w-[55%] xl:w-[60%] relative pl-4 md:pl-8">
              <div className="relative w-full h-75 xl:h-95 rounded-xl md:rounded-3xl overflow-hidden shadow-2xl bg-[#111] border border-white/10">
                {services.map((service, i) => (
                  <div
                    key={`img-${service.id}`}
                  ...
                  >
                    ...
                    {/* Gradient Overlays */}
                    <div className="absolute inset-0 bg-linear-to-t from-black/80 via-transparent to-black/10 mix-blend-multiply opacity-60"></div>
                    ...
                    <div className="absolute bottom-8 md:bottom-10 left-8 md:left-10 right-8 md:right-10">
                      <h3 className={`font-display text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-white tracking-tight leading-none transition-all duration-700 delay-100 ${i === activeIndex ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                        {service.headline}
                      </h3>
                    </div>
                  </div>
